...
![AI Product Intelligence Suite hero](assets/screenshots/01_hero_demo_start_here.png)
...
2. Keep **Demo Mode ON**.
3. Open **Start Here — Hero Demo**.
4. Follow the hero workflow: source → obligations → review → requirement → QA → release → audit.
...
| Area | What it demonstrates | Why it matters |
|---|---|---|
| Interpret | Source-to-obligation extraction with evidence | Keeps compliance input traceable |
| Review | Human reviewer corrections | Prevents blind trust in AI output |
| Discover | Requirements, tickets and PRD checks | Turns obligations into product work |
| QA | Positive and negative test coverage | Catches failure modes before release |
| Communicate | Risky claim detection and safer rewrites | Reduces customer-facing overclaim risk |
| Investigate | Incident timeline and decision log | Supports postmortem and audit learning |
| Governance | Real vs simulated capability labels | Prevents portfolio overclaiming |
...
- [`HERO_CASE_STUDY.md`](HERO_CASE_STUDY.md) — hero case summary.
